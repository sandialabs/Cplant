...
int lwpflag = 0;	/* Support for LWP */
int ptflag = 0;		/* Support POSIX threads instead of LWP */
#define	INLINSZ 5
/* length at which to start doing an inline */

int inlinsz = INLINSZ;
/*
 * Length at which to start doing an inline. INLINSZ = default
...
static void c_output();
static void clnt_output();
static int do_registers();
static void h_output();
static void l_output();
	...
	char *argv[];
{
	...
	if (!parseargs(argc, argv, &cmd))
		usage();
	if (lwpflag && ptflag) {
		f_print(stderr, "Only one of -M or -P allowed.\n");
		crash();
	}
...
static int cpp_search(s, path, pathsiz)
	char	*s;
	char	*path;
	unsigned pathsiz;
{
...
static void find_cpp()
{
	...
	/* explicit cpp exist? */
	if (cppDefined && !cpp_search(CPP, path, sizeof(path)) != 0) {
		fprintf(stderr,
	...
	if (s == NULL)
		s = "/bin:/usr/bin";
	if (cpp_search(s, path, sizeof(path)))
		return;

	if (cpp_search("/lib:/usr/lib", path, sizeof(path)))
		return;
	...
	int netflag;
{
	...
	f_print(fout, "#include <string.h> /* memset */\n");
	if (Cflag) {
#ifndef UNFS
		f_print (fout,
	...
	char *outfile;
{
	...
	if (infile && (include = extendfile(infile, ".h"))) {
		...
		f_print(fout, "#include <rpc/rpc.h>\n");
#else
		f_print(fout, "#include \"rpc/rpc.h\"n");
#endif
	f_print(fout, "#include <stdio.h>\n");
	if (Cflag)
	...
	struct commandline *cmd;
{
	...
	nflags = cmd->cflag + cmd->hflag + cmd->lflag +
			cmd->mflag + cmd->sflag + cmd->nflag + cmd->tflag +
...
usage()
{
	...
	f_print(stderr,
		"\t%s [-abCLNT{M|P}] [-Dname[=value]] [-i size]\
...
inactivity\n");
	...
	f_print(stderr, "-M\t\tgenerate MT-safe code (Sun LWP)\n");
