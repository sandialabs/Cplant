...
static void write_inetmost();
static void write_pm_most();
static void write_rpc_svc_fg();
static void print_pmapunset();
static void open_log_file();
static void print_err_message();
static void write_real_program();
...
serviced */\n");
	...
	if (inetdflag) {
		...
		if (tirpcflag) {
			...
			if (lwpflag & timerflag)
				f_print(fout, "\tassert(mutex_init(&_svcstate_lock, USYNC_THREAD, NULL) = 0);\n");
	...
	char *storage;
{
	...
	for (vp = def->def.pr.versions; vp != NULL; vp = vp->next) {
		...
		if (timerflag) {
			if (lwpflag)
				f_print(fout, "\tassert(mutex_lock(&_svcstate_lock) == 0);\n");
			else if (ptflag)
				f_print(fout, "\tassert(pthread_mutex_lock(&_svcstate_lock) == 0);\n");

			f_print(fout, "\t_rpcsvcstate = _SERVING;\n");				
			if (lwpflag)
				f_print(fout, "\tassert(mutex_unlock(&_svcstate_lock) == 0);\n");
			else if (ptflag)
				f_print(fout, "\tassert(pthread_mutex_unlock(&_svcstate_lock) == 0);\n");
		}
	...
	char *space;
{
	if (exitnow)
		...
		if (timerflag) {
			if (lwpflag)
				f_print(fout, "%sassert(mutex_lock(&_svcstate_lock) == 0);\n", space);
			else if (ptflag)
				f_print(fout, "%sassert(pthread_mutex_lock(&_svcstate_lock) == 0);\n", space);
			f_print(fout, "%s_rpcsvcstate = _SERVED;\n", space);
			if (lwpflag)
				f_print(fout, "%sassert(mutex_unlock(&_svcstate_lock) == 0);\n", space);
			else if (ptflag)
				f_print(fout, "%sassert(pthread_mutex_unlock(&_svcstate_lock) == 0);\n", space);
		}
