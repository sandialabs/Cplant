...
static void emit_union();
static void emit_enum();
static void emit_struct();
static void emit_typedef();
static void print_trailer();
static void print_stat();
static void emit_inline();
static void emit_single_in_line();
	...
	definition *def;
{
	...
	f_print(fout, "\tregister xdr_unit_t *buf;\n\n");
}
	...
	definition *def;
{
	...
	if (dflt != NULL && !streq(dflt->type, "void")) {
...
int flag;
{
	...
	if (i > 0) {
		if (sizestr == NULL && size < inlinsz){
			/* don't expand into inline code if size < inlinsz */
			while (cur != dl){
				print_stat(indent + 1, &cur->decl);
				cur = cur->next;
			}
