...
**************************************************************************/
/*
** $Id: srvr_err.c,v 1.7 2001/02/11 13:08:47 lafisk Exp $
**
...
char buf[1024];    
    ...
    if (errnoflag&CP_ERRNO){       /* we set CPerrno */
        ...
        sprintf(buf + strlen(buf), " (srvr lib: %s)", CPstrerror(CPerrno));
    }

    if (errnoflag& P30_ERRNO){  /* we set srvr_p30errno to p30 lib return value*/

        sprintf(buf + strlen(buf), " (p30 lib: %s)", p30strerror(srvr_p30errno));
    }
