...
**************************************************************************/
/*
** $Id: srvr_coll_membership.c,v 1.1 2000/11/04 03:56:31 lafisk Exp $
*/
#include "puma.h"
...
char *newPtr;
    ...
    if ((maxm >= TOO_MANY_MAX_MEMBERS) || (maxm < 0) ||
	(myRealRank < 0) || (myRealRank >= maxm) ||
	dsrvrMembersInited){
...
#else
    MemberList.mid[myRealRank].pid = _my_pid;
#endif /* PPID */
...
** leave the group due to our own resource problems.)
*/
int
...
int i, groupRank, rc, rr;
    ...
    */
    ...
    /*
    ** We'll synchronize on the SYS_CTRL_PTL set up before main.  Once
    ** out successfully, the group is valid.
    */

    rc = dsrvr_system_synchronization(2,        /* fanout tree degree */
			      dsrvrGroupId,   /* use this for msg type */
    ...
    /*
    ** Reset all portals used for group communication.
    */
    dsrvr_comm_reset();
    ...
    */
    ...
    rc = dsrvr_system_synchronization(2,
			      dsrvrGroupId,
			      timeout);
