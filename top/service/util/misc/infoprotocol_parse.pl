#!/usr/bin/perl -w
#
# $Id: infoprotocol_parse.pl,v 1.1.2.1 2002/06/18 18:37:27 jbogden Exp $
#
# Wraps the infoprotocol utility and does post processing so that meaningful
# data is easily seen.
...
GetOptions( 'h|U|help' => \$help,
            'sleep=i' => \$sleep_time,
            'p|packets' => \$packetstats
          );

if (defined $help) {
    print "USAGE: $0 [-sleep <seconds>] [-packets]\n";
    print "           -sleep    number of seconds to sleep between queries\n";
    print "           -packets  process all packet level counters\n";
    exit 1;
}
...
$num_summary_lines = 13;
$num_node_info_lines = 11;
...
%field_to_name = (
    ...
    3 => "orphan CTS",
    4 => "orphan Data",
    5 => "Last pkt != LAST_DATA",
    6 => "GCH done unsuccessful",
    7 => "Wrong msg num dropped",
    8 => "Pkts out of sequence",
    9 => "orphan P3_RESEND",
   10 => "orphan DATA_RESEND",
   11 => "Unxpctd evnt Qsnd_pnd",
   12 => "Unxpctd evnt Qsending",
   13 => "Ill. LstEvnt Qrcving",
   14 => "P3 resend failed",
   15 => "P3 resend OK",
   16 => "P3 RTS timeout",
   17 => "P3 seq sync attempted",
   18 => "P3 seq sync accepted",
   19 => "RTSCTS packets sent",
   20 => "RTSCTS packets recvd"
);
...
@field_to_index = (0,3,6,-1, 3,6,-1, 5,9,-1, 5,10,-1, 3,6,-1, 4,8,-1, 4,8,-1, 4,8,-1, 5,10,-1, 4,8,-1, -1);
...
for $node (sort { $a <=> $b } (keys(%hash1)) ) {
    ...
    for ($i = 1; $i < $num_fields; $i++) {
        if (!defined($packetstats) and ($i == 19 or $i == 20)) {
            next;
        }
