...
#############################################################################
**************************************************************************/
/* $Id: CMDhandler_rmdir.c,v 1.6 2001/02/16 05:44:25 lafisk Exp $ */
...
VOID 
CMDhandler_rmdir( control_msg_handle *mh)
{
hostReply_t   ack;
...
int gid_real = getgid();
    ...
    {
        fprintf(stderr,"CMDhandler_rmdir() nid %i pid %i\n", snid, spid);
    }
    ...
    if ( iAmFyod() ) {
      if (cmd->fail == 1) {
        ...
        memmove(fname+6, fname, cmd->info.rmdirCmd.fnameLen);
