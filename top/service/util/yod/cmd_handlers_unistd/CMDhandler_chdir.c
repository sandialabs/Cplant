...
#############################################################################
**************************************************************************/
/* $Id: CMDhandler_chdir.c,v 1.7 2001/02/16 05:44:25 lafisk Exp $ */
...
VOID 
CMDhandler_chdir( control_msg_handle *mh)
{
hostReply_t ack;
INT32        retVal;
...
hostCmd_t   *cmd;
    ...
    {
        fprintf(stderr,"CMDhandler_chdir() nid %i pid %i\n", snid, spid);
    }
    ...
    if ( iAmFyod() ) {
      if (cmd->fail == 1) {
        ...
        memmove(fname+6, fname, cmd->info.chdirCmd.fnameLen);
