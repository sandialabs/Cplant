...
#############################################################################
**************************************************************************/
/* $Id: CMDhandler_mkdir.c,v 1.7 2001/02/16 05:44:25 lafisk Exp $ */
...
VOID 
CMDhandler_mkdir(control_msg_handle *mh)
{
hostReply_t ack;
...
hostCmd_t   *cmd;
    ...
    {
        fprintf(stderr,"CMDhandler_mkdir() nid %i pid %i\n", snid, spid);
    }
    if (len > MAXPATHLEN){
        ioErrno = EPROTOCOL;
        goto done;
    ...
    if ( iAmFyod() ) {
      if (cmd->fail == 1) {
        ...
        memmove(fname+6, fname, cmd->info.mkdirCmd.fnameLen);
    ...
    ack.retVal = mkdir( fname, cmd->info.mkdirCmd.mode );
    ack.hostErrno = errno;
