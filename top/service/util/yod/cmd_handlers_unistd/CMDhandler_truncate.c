...
#############################################################################
**************************************************************************/
/* $Id: CMDhandler_truncate.c,v 1.6 2001/02/16 05:44:25 lafisk Exp $ */
...
VOID 
CMDhandler_truncate(control_msg_handle *mh)
{
hostReply_t ack;
INT32        retVal;
...
int gid_real = getgid();
    ...
    {
        fprintf(stderr,"CMDhandler_truncate() nid %i pid %i\n", snid, spid);
    }
    if (len > MAXPATHLEN){
        ioErrno = EPROTOCOL;
        goto done;
    ...
    if ( iAmFyod() ) {
      if (cmd->fail == 1) {
        ...
        memmove(fname+6, fname, cmd->info.truncateCmd.fnameLen);
    ...
    retVal = truncate( fname, cmd->info.truncateCmd.length );
