...
#############################################################################
**************************************************************************/
/* $Id: CMDhandler_chown.c,v 1.6 2001/02/16 05:44:25 lafisk Exp $ */
...
VOID 
CMDhandler_chown( control_msg_handle *mh)
{
hostReply_t ack;
INT32        retVal;    
...
int gid_real = getgid();
    ...
    {
        fprintf(stderr,"CMDhandler_chown() nid %i pid %i\n", snid, spid);
    }
	...
	send_ack_to_app(mh, &ack);
        ...
        memmove(fname+6, fname, cmd->info.chownCmd.fnameLen);
    ...
    retVal = chown(fname, cmd->info.chownCmd.owner, cmd->info.chownCmd.group);
