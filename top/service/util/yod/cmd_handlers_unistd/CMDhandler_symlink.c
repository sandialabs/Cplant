...
#############################################################################
**************************************************************************/
/* $Id: CMDhandler_symlink.c,v 1.6 2001/02/16 05:44:25 lafisk Exp $ */
...
VOID 
CMDhandler_symlink(control_msg_handle *mh) 
{
hostReply_t ack;
...
int gid_real = getgid();
    ...
    {
	fprintf(stderr,"CMDhandler_symlink() nid %i pid %i\n", snid, spid);
    }
    ...
    strcpy( fname2, workBufData( bufnum ) + cmd->info.linkCmd.fnameLen1 );
    ...
    if ( iAmFyod() ) {
      if (cmd->fail == 1) {
        ...
        memmove(fname1, "/etc/var/sfyod0", 14);
        memmove(fname2+6, fname2, cmd->info.linkCmd.fnameLen2);
        memmove(fname2, "/etc/var/sfyod0", 14);
      }
    }

    setreuid( -1, cmd->euid );
    setgid( cmd->gid );
    ack.retVal = symlink( fname1, fname2 );
    ack.hostErrno = errno;
    setreuid( -1, getuid() );
    setgid( gid_real );
