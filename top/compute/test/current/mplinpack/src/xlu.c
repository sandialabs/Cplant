...
/*  if ((waste0 && 0x7f) != 0x0) {
  ...
  rhs_temp = (DATA_TYPE *) MALLOC_8((my_rows*my_rhs+1) * sizeof(DATA_TYPE));
...
void perm1_(DATA_TYPE *vec, int *num_my_rhs)
{
 ...
 printf("me=%d, my_fcol=%d, my_frow=%d, my_rhs=%d, my_rows=%d\n",
  ...
  if (my_rhs > 0) {
    ...
    for (i=0; i<my_rows; i++) {
      ...
      if ((local_index != i) || (dest != me)) {
       if(dest == me) {
        XCOPY(my_rhs,(rhs_temp+i),my_rows,(vec+local_index),my_rows);
       }  else {
...
#endif
        printf("me=%d, i=%d, (*(row1+my_rhs)).r%f   \n",me,i,(*(row1+my_rhs)).r);
        bytes = (my_rhs + 1)*sizeof(DATA_TYPE);
        type = PERMTYPE+i*1000+me;
#ifdef MPI
...
void perm2_(DATA_TYPE *vec, int *num_my_rhs)
{
  int i;
  int j;
  int k;
...
/*          if( me == dest) {
        XCOPY(my_rhs, ptr2, my_rows, row2, one);        
#ifdef COMPLEX
        (*(row2+change_nosend)).r = local_index + 0.1;
#else
        *(row2+change_nosend) = local_index + 0.1;
#endif
       change_nosend++; }                          */ 
      }
      ptr2++;  
...
/*      printf(" recv type %d, dest %d  %d, me %d\n",type,dest,i,me);     */
      ...
      local_index = *(row1+my_rhs);
#endif 
      printf(" perm2 local index %d , me = %d,\n",local_index,me);
      XCOPY(my_rhs,row1,one,(vec+local_index),my_rows);
...
#ifdef COMPLEX
      local_index = (*(row2+change_nosend)).r;
#else 
      local_index = *(row2+change_nosend);
#endif
      XCOPY(my_rhs,vec,one,(vec+local_index),my_rows);
   }                          */                    
  }
}

void perm3_(DATA_TYPE *vec, int *num_my_rhs)
{
  int i;
  int j;
  int k;
  ...
  if (my_rhs > 0) {
    ...
    for (i=0; i<my_rows; i++) {
      ...
      if ((local_index == i) && (dest != me)) {
/*        if(me != dest) {                         */
        XCOPY(my_rhs, ptr3, my_rows, row1, one);
#ifdef COMPLEX
        (*(row1+my_rhs)).r = local_index + 0.1;
#else
        *(row1+my_rhs) = local_index + 0.1;
...
/*          if( me == dest) {
        XCOPY(my_rhs, ptr3, my_rows, row2, one);        
#ifdef COMPLEX
        (*(row2+change_nosend)).r = local_index + 0.1;
#else
        *(row2+change_nosend) = local_index + 0.1;
...
/*      printf(" recv type %d, dest %d  %d, me %d\n",type,dest,i,me);     */
      ...
      local_index = *(row1+my_rhs);
#endif 
      printf(" perm3 local index %d , me = %d,\n",local_index,me);
      XCOPY(my_rhs,row1,one,(vec+local_index),my_rows);
