...
/* usage: 
   % rename [-yod | -fyod | -enfs]
*/
static const char* TNAME = "rename";
...
int main (int argc, char* argv[])
{
        ...
        while( ch != EOF  && valid_option==0  ) {
          ...
          switch(ch) {
            case 'e':
              ...
              mvd_file     = "enfs:/enfs/tmp/io_mvd_file";
              valid_option = 1;
              break;

            case 'y':
              default_file = "./io_test_file";
              mvd_file     = "./io_mvd_file";
              valid_option = 1;
              break;

            case 'f':
              default_file = "/raid_010/tmp/io_test_file";
              mvd_file     = "/raid_010/tmp/io_mvd_file";
              valid_option = 1;
              break;
	...
	printf("%s: file to move to, %s\n", TNAME, mvd_file);
        ...
        */
        ...
        if ( rc < 0 ) {
          if ( errno != ENOENT ) {
            printf("%s: FAILED T3 unlink(mvd_file) test\n", TNAME);
            exit(-1);
          }
        }
        printf("%s: PASSED T3 unlink(mvd_file) test\n", TNAME);

        /* 4) use access to verify mvd file does not exist... */

        rc = access(mvd_file, F_OK);
        if ( rc == 0 ) {
          printf("%s: FAILED T4 access(mvd_file,~F_OK) test -- %s should NOT exist\n", TNAME, mvd_file);
          exit(-1);
        }
        printf("%s: PASSED T4 access(mvd_file,~F_OK) test -- %s does not exist\n", TNAME, mvd_file);
...
/*--------------------------------------------------------------------------*/
        ...
        if ( rc < 0 ) {
	  printf("%s: FAILED T7 rename() test, call failed\n", TNAME);
          exit(-1);
        }

        printf("%s: PASSED T7 rename() test\n", TNAME);
