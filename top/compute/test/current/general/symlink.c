...
/* usage: 
   % symlink [-yod | -fyod | -enfs]
*/
static const char* TNAME = "symlink";
...
/*--------------------------------------------------------------------------*/
        ...
        /* 7) symlink() */
        rc = symlink(default_file, link_file);
        if ( rc < 0 ) {
	  printf("%s: FAILED T7 symlink() test, call failed\n", TNAME);
          exit(-1);
        }

        printf("%s: PASSED T7 symlink() test\n", TNAME);
