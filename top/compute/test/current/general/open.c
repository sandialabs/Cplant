...
int main (int argc, char* argv[])
{
        ...
        while( ch != EOF  && valid_option==0  ) {
          ...
          switch(ch) {
            ...
            case 'h':
              printf("open: valid options are -yod or -fyod or -enfs\n");
              exit(0);
              break;
        ...
        if ( valid_option != 1) {
          printf("open: bad command line options,\n");
          printf("open: valid options are -yod or -fyod or -enfs\n");
          exit(0);
        }

	printf("open: file to test: %s\n", default_file);
        ...
        if ( fd < 0 ) {
	  printf("open: FAILED open() test, call failed... %s\n",strerror(errno));
          exit(-1);
        }
	printf("open: open() test SUCCEEDED...\n");

        /* write() */
        rc = write(fd,default_str,strlen(default_str));
        if ( rc != strlen(default_str)) {
          printf("open: FAILED write(), call failed... %s\n",strerror(errno));
          exit(-1);
        }
        printf("open: write() test SUCCEEDED...\n");

        rc = close(fd);
        if ( rc < 0 ) {
          printf("open: close() FAILED... %s\n",strerror(errno));
          exit(-1);
        }
        printf("open: close() SUCCEEDED...\n");
