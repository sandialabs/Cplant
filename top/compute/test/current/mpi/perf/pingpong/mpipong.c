...
**************************************************************************/
/*
** $Id: mpipong.c,v 1.1 1997/10/29 20:32:34 bright Exp $
**
** Measure the latency for a zero length message.
...
*/   
    ...
    while ((ch= getopt(argc, argv, "l:n:")) != EOF)   {
        switch (ch)   {
            case 'l': len= strtol(optarg, (char **)NULL, 0);
                      break;
            case 'n': trials= strtol(optarg, (char **)NULL, 0);
                      break;
            default:  error= TRUE;
        }
    }
    ...
    if (error)   {
        if (my_node == 0)   {
            fprintf(stderr, "Usage: %s [-l length] [-n trials]\n", argv[0]);
        }
        exit (-1);
    }
    ...
    if (my_node == 0)   {
	printf("\n");
	printf("Results for %d trials of %d length each\n\n", trials, len);
	printf("                 minimum     average     maximum\n");
	printf("Latency:        %8.2f    %8.2f    %8.2f    micro seconds\n",
	    min_latency, tot_latency / trials, max_latency);

	printf("Bandwidth:      %8.2f    %8.2f    %8.2f    mega bytes per second\n",
	    min_bandwidth / (1024 * 1024), 
	    (tot_bandwidth / trials) / (1024 * 1024), 
	    max_bandwidth / (1024 * 1024));
	printf("Bandwidth:      %8.2f    %8.2f    %8.2f    million bytes per second\n",
