	  ...
	  unsigned long arg)
{
	...
	case NFSGETFH:
		/*
		 * Return the NFS file handle associated with this descriptor/
		 * inode. This call is used by ROMIO to enable job-wide
		 * locking.
		 */
		if((struct nfs_fh *)arg == NULL)
			err = -EINVAL;
		else
			err = copy_to_user((struct nfs_fh *)arg,
					   NFS_FH(dentry),
					   sizeof(struct nfs_fh)) ? -EFAULT : 0;
		break;
	default:
		return -ENOTTY;
	}
