...
/***************************************************************************
  ...
  ***************************************************************************/
int MPID_P31_Long_send( buf, len, src_lrank, tag, context_id, dest,
				    msgrep, comm )
void *buf;
...
struct MPIR_COMMUNICATOR *comm;
{
    ...
    if ( MPID_DebugFlag ) {
        P31_Printf("entering MPID_P31_Long_send()\n");
    }
#endif

    if ( (mpi_errno = MPID_P31_Long_isend( buf, len, src_lrank, tag, 
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("calling MPID_P31_Long_wait_send()\n");
    }
#endif
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("leaving MPID_P31_Long_send()\n");
    }
#endif
...
/***************************************************************************
  ...
  ***************************************************************************/
int MPID_P31_Long_isend( buf, len, src_lrank, tag, context_id, dest,
			   msgrep, shandle, comm )
void *buf;
int  len, tag, context_id, src_lrank, dest, msgrep;
MPIR_SHANDLE *shandle;
...
#   if defined(MPID_DEBUG_ALL) || defined(MPID_DEBUG_SEND)
    if (MPID_DebugFlag) {
        P31_Printf("MPID_P31_Long_isend\n");
    }
#   endif
...
#   if defined(MPID_DEBUG_ALL) || defined(MPID_DEBUG_SEND)
	...
	P31_Printf("    _mpi_p31_read_me_handle = %d\n",_mpi_p31_read_me_handle);
	P31_Dump_process_id( &dest_id );
	P31_Printf("    match_bits              = 0x%lx\n",match_bits);
	P31_Printf("    acl                     = 0\n");
	P31_Printf("    link                    = PTL_RETAIN\n");
	P31_Printf("    insert                  = PTL_INS_AFTER\n");
	P31_Printf("    &shandle->me_handle     = %p )\n",&shandle->me_handle);
    }
#   endif

    /* get an me on the read portal */
    P31_CALL( PtlMEInsert( _mpi_p31_read_me_handle,
			   dest_id,
			   match_bits,
			   0,
			   PTL_RETAIN,
...
#   if defined(MPID_DEBUG_ALL) || defined(MPID_DEBUG_SEND)
	...
	P31_Printf("     shandle->me_handle     = %d\n",shandle->me_handle);
	P31_Dump_md( &md );
	P31_Printf("    link                   = PTL_RETAIN\n");
	P31_Printf("    &shandle->md_handle    = %p )\n",&shandle->md_handle);
    }
#   endif
    ...
    P31_SET_LONG_SEND_BITS( match_bits, context_id, src_lrank, tag );
...
#   endif
    ...
    shandle->wait	     = MPID_P31_Long_wait_send;
    shandle->test	     = MPID_P31_Long_test_send;

#   if defined(MPID_DEBUG_ALL)
    if ( MPID_DebugFlag ) {
        P31_Printf("leaving MPID_P31_Long_isend()\n");
    }
#   endif
...
/***************************************************************************
  ...
  MPID_P31_Long_send_cleanup

  ***************************************************************************/
void MPID_P31_Long_send_cleanup( shandle )
MPIR_SHANDLE *shandle;
{

#   if defined(MPID_DEBUG_ALL)
    if ( MPID_DebugFlag ) {
        P31_Printf("entering MPID_P31_Long_send_cleanup()\n");
    }
#   endif

#   if defined(MPID_DEBUG_ALL)
    if ( MPID_DebugFlag ) {
	P31_Printf("  calling PtlMEUnlink( shandle->me_handle = %d )\n",shandle->me_handle);
    }
#   endif

    /* free the me handle (which frees up the md too) */
    P31_CALL( PtlMEUnlink( shandle->me_handle ), PtlMEUnlink );

#   if defined(MPID_DEBUG_ALL)
    if ( MPID_DebugFlag ) {
	P31_Printf("  calling MPID_P31_Free_eq_handle( shandle->eq_handle_list_ptr = %p )\n",
...
#   if defined(MPID_DEBUG_ALL)
    if ( MPID_DebugFlag ) {
        P31_Printf("leaving MPID_P31_Long_send_cleanup()\n");
    }
#   endif
...
/***************************************************************************
  ...
  MPID_P31_Long_test_send

  test to see if the kernel has processed the send

  ***************************************************************************/
int MPID_P31_Long_test_send( shandle )
MPIR_SHANDLE *shandle;
{
    int         mpi_errno = MPI_SUCCESS;
    ptl_event_t event;
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("entering MPID_P31_Long_test_send()\n");
    }
#endif
...
#       endif
	...
	if ( shandle->got_sent && shandle->got_ack ) {
		...
		MPID_P31_Long_send_cleanup( shandle );
		
	    } else {
		/* message wasn't received - check for read */
		shandle->wait = MPID_P31_Long_wait_send_read;
		shandle->test = MPID_P31_Long_test_send_read;
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("leaving MPID_P31_Long_test_send()\n");
    }
#endif
...
/***************************************************************************
  ...
  MPID_P31_Long_test_send_read

  test for reading of the send buffer

  ***************************************************************************/
int MPID_P31_Long_test_send_read( shandle )
MPIR_SHANDLE *shandle;
{
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("entering MPID_P31_Long_test_send_read()\n");
    }
#endif
...
#       endif
	...
	if ( event.type != PTL_EVENT_GET ) {
	    P31_Printf("ERROR: was expecting a PTL_EVENT_GET\n");
	    P31_Dump_event( &event );
	}
    ...
    if ( MPID_DebugFlag ) {
	    ...
	    P31_Printf("request is complete\n");
	}
	else {
	    P31_Printf("request is not complete\n");
	}
        P31_Printf("leaving MPID_P31_Long_test_send_read()\n");
    }
#endif
...
/***************************************************************************
  ...
  MPID_P31_Long_wait_send

  wait for completion of long protocol send

  ***************************************************************************/
int MPID_P31_Long_wait_send( shandle )
MPIR_SHANDLE *shandle;
{
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("entering MPID_P31_Long_wait_send()\n");
    }
#endif
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("leaving MPID_P31_Long_wait_send()\n");
    }
#endif
...
/***************************************************************************
  ...
  MPID_P31_Long_wait_send_read

  wait for the send buffer to be read

  ***************************************************************************/
int MPID_P31_Long_wait_send_read( shandle )
MPIR_SHANDLE *shandle;
{
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("entering MPID_P31_Long_wait_send_read()\n");
    }
#endif

    if ( (_mpi_p31_errno = PtlEQWait( shandle->eq_handle_list_ptr->eq_handle, &event )) != PTL_OK ) {
	fprintf(stderr,"%d: (%s:%d) PtlEQWait() failed : %s\n",
		MPID_MyWorldRank, __FILE__, __LINE__, ptl_err_str[_mpi_p31_errno]);
    }
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("leaving MPID_P31_Long_wait_send_read()\n");
    }
#endif
...
/***************************************************************************
  ...
  MPID_P31_Long_delete

  cleanup

  ***************************************************************************/
void MPID_P31_Long_delete( MPID_Protocol *p )
{

#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("entering MPID_P31_Long_delete()\n");
    }
#endif
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("leaving MPID_P31_Long_delete()\n");
    }
#endif
...
/***************************************************************************
  ...
  ***************************************************************************/
MPID_Protocol *MPID_P31_Long_setup()
{
    MPID_Protocol *p;

#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("entering MPID_P31_Long_setup()\n");
    }
#endif

    p = (MPID_Protocol *) MALLOC( sizeof(MPID_Protocol) );
    if (!p) return 0;
    p->send	   = MPID_P31_Long_send;
    p->recv	   = 0;
    p->isend	   = MPID_P31_Long_isend;
    p->ssend	   = MPID_P31_Long_send;
    p->issend	   = MPID_P31_Long_isend;
    p->wait_send   = 0;
    p->push_send   = 0;
    p->cancel_send = 0;
    p->irecv	   = 0;
    ...
    p->do_ack      = 0;
    p->unex        = 0;
    p->delete      = MPID_P31_Long_delete;

#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("leaving MPID_P31_Long_setup()\n");
    }
#endif
