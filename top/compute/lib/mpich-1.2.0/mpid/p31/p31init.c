...
/***************************************************************************
  ...
  MPID_P31_InitMsgPass

  ***************************************************************************/
MPID_Device *MPID_P31_InitMsgPass( argc, argv, short_len, long_len )
int  *argc;
char ***argv;
int  short_len, long_len;
{
    ...
    if ( MPID_DebugFlag ) {
        P31_Printf("entering MPID_CH_InitMsgPass()\n");
    }
#endif

    /* get the length of a long protocol message */
    if ( (tmpstring = getenv(MPI_ENV_LONG_MSG_STRING)) ) {
	_mpi_p31_short_size = atoi(tmpstring);
    }
    if ( _mpi_p31_short_size < 0 ) {
	_mpi_p31_short_size = P31_DEFAULT_LONG_MSG;
    }

    short_len = _mpi_p31_short_size;
    ...
    dev->short_msg    = MPID_P31_Short_setup();
    dev->long_msg     = MPID_P31_Long_setup();
    dev->vlong_msg    = dev->long_msg;
    dev->ready        = MPID_P31_Ready_setup();
    dev->check_device = 0;
    dev->terminate    = MPID_P31_End;
    dev->abort	      = MPID_P31_Abort;
    dev->next	      = 0;
    ...
    if ( MPID_P31_init( argc, *argv ) ) {

#ifdef MPID_DEBUG_ALL
      if ( MPID_DebugFlag ) {
          P31_Printf("Error initializing portals device\n");
      }
#endif
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("leaving MPID_CH_InitMsgPass()\n");
    }
#endif
...
/***************************************************************************
  ...
  ***************************************************************************/
int MPID_P31_Abort( comm, code, msg )
...
/***************************************************************************
  ...
  ***************************************************************************/
int MPID_P31_End( dev )
MPID_Device *dev;
{

#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("entering MPID_P31_End()\n");
    }
#endif

    /* Finish off any pending transactions */
    /* MPID_P31_Complete_pending(); */
...
/***************************************************************************
  ...
  MPID_P31_Version_name

  ***************************************************************************/
void MPID_P31_Version_name( name )
char *name;
{
