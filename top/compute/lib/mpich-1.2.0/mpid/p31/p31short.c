...
/***************************************************************************
  ...
  ***************************************************************************/
int MPID_P31_Short_send( buf, len, src_lrank, tag, context_id, dest,
				    msgrep, comm )
void *buf;
...
struct MPIR_COMMUNICATOR *comm;
{
    ...
    if ( MPID_DebugFlag ) {
        P31_Printf("entering MPID_P31_Short_send()\n");
    }
#endif

    if ( (mpi_errno = MPID_P31_Short_isend( buf, len, src_lrank, tag, 
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("waiting for send to complete\n");
    }
#endif
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("leaving MPID_P31_Short_send()\n");
    }
#endif
...
/***************************************************************************
  ...
  MPID_P31_Short_isend
  
  nonblocking short protocol send

  ***************************************************************************/
int MPID_P31_Short_isend( buf, len, src_lrank, tag, context_id, dest,
			   msgrep, shandle, comm )
void *buf;
int  len, tag, context_id, src_lrank, dest, msgrep;
MPIR_SHANDLE *shandle;
...
#   if defined(MPID_DEBUG_ALL) || defined(MPID_DEBUG_SEND)
    if (MPID_DebugFlag) {
        P31_Printf("P31_Post_send_short\n");
	P31_Printf("  buf        = %p\n",buf);
	P31_Printf("  len        = %d\n",len);
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("leaving MPID_P31_Short_isend()\n");
    }
#endif
...
/***************************************************************************
  ...
  MPID_P31_Short_bsend
  
  nonblocking short protocol buffered send

  ***************************************************************************/
int MPID_P31_Short_bsend( buf, len, src_lrank, tag, context_id, dest,
			   shandle, comm )
void *buf;
...
struct MPIR_COMMUNICATOR *comm;
{
    ...
    MPID_P31_Short_isend( buf, len, src_lrank, tag, context_id, dest,
			 0, shandle, comm );

    return MPID_P31_Short_wait_send( shandle );
...
/***************************************************************************
  ...
  MPID_P31_Short_test_send

  test for the completion of short protocol send request

  ***************************************************************************/
int MPID_P31_Short_test_send( shandle )
MPIR_SHANDLE *shandle;
{
    ptl_event_t event;
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("entering MPID_P31_Short_test_send()\n");
	P31_Printf("  calling PtlEQGet(\n");
	P31_Printf("    eq_handle = %d\n",shandle->eq_handle_list_ptr->eq_handle);
	P31_Printf("    &event    = %p )\n",&event);
    }
#endif
...
#       if defined(MPID_DEBUG_ALL)
	if ( MPID_DebugFlag ) {
	    P31_Printf("  got an event");
	    P31_Dump_event( &event );
	    P31_Printf("calling MPID_P31_Free_eq_handle( ptr = %p )\n",
		       shandle->eq_handle_list_ptr);
	    P31_Printf("send with handle %p is done\n",shandle);
	}
#       endif
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
	P31_Printf("leaving MPID_P31_Short_test_send()\n");
    }
#endif
...
/***************************************************************************
  ...
  MPID_P31_Short_wait_send

  wait for the completion of short protocol send request

  ***************************************************************************/
int MPID_P31_Short_wait_send( shandle )
MPIR_SHANDLE *shandle;
{
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("entering MPID_P31_Short_wait_send()\n");
	P31_Printf("  calling PtlEQWait(\n");
	P31_Printf("    eq_handle = %d\n",shandle->eq_handle_list_ptr->eq_handle);
	P31_Printf("    &event    = %p )\n",&event);
    }
#endif
	...
	P31_Printf("ERROR: was expecting a PTL_EVENT_SENT\n");
	P31_Dump_event( &event );
    }
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
	P31_Printf("leaving MPID_P31_Short_wait_send()\n");
    }
#endif
...
/***************************************************************************
  ...
  ***************************************************************************/
void MPID_P31_Short_delete( MPID_Protocol *p )
{

#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("entering MPID_P31_Short_delete()\n");
    }
#endif
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("leaving MPID_P31_Short_delete()\n");
    }
#endif
...
MPID_Protocol *MPID_P31_Short_setup()
{
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P31_Printf("entering MPID_P31_Short_setup()\n");
    }
#endif

    p = (MPID_Protocol *) MALLOC( sizeof(MPID_Protocol) );
    if (!p) return 0;
    p->send	   = MPID_P31_Short_send;
    p->recv	   = 0;
    p->isend	   = MPID_P31_Short_isend;
    p->ssend	   = MPID_P31_Sshort_send;
    p->issend	   = MPID_P31_Sshort_isend;
    p->wait_send   = 0;
    p->push_send   = 0;
    p->cancel_send = 0;
    p->irecv	   = 0;
    ...
    p->do_ack      = 0;
    p->unex        = 0;
    p->delete      = MPID_P31_Short_delete;
