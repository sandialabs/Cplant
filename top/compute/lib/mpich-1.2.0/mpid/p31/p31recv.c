...
/***************************************************************************
  ...
  ***************************************************************************/
int MPID_P31_Test_recv( MPIR_RHANDLE *rhandle )
{
...
#   if defined(MPID_DEBUG_ALL)
    if ( MPID_DebugFlag ) {
	P31_Printf("entering P31_Test_recv()\n");
...
#   endif
    ...
    if ( (_mpi_p31_errno = PtlEQGet( rhandle->eq_handle_list_ptr->eq_handle,
				     &event )) == PTL_OK ) {
...
#           if defined(MPID_DEBUG_ALL)
	    if ( MPID_DebugFlag ) {
		P31_Printf("  message was truncated\n");
	    }
#           endif
...
#       if defined(MPID_DEBUG_ALL)
	if ( MPID_DebugFlag ) {
	    ...
	    P31_Printf("recv with handle %p is done\n",rhandle);
	}
#       endif
	...
	if ( MPID_DebugFlag ) {
	    P31_Printf("leaving P31_Test_recv()\n");
	}
#endif
...
/***************************************************************************
  ...
  ***************************************************************************/
int MPID_P31_Wait_recv( MPIR_RHANDLE *rhandle )
{
    int mpi_errno = MPI_SUCCESS;
    ptl_event_t event;
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
	P31_Printf("entering P31_Wait_recv()\n");
    ...
    if ( (_mpi_p31_errno = PtlEQWait( rhandle->eq_handle_list_ptr->eq_handle, &event )) != PTL_OK ) {
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
	P31_Printf("leaving P31_Wait_recv()\n");
    }
#endif
...
/***************************************************************************
  ...
  MPID_P31_Test_pulled
  
  Check to see if a pulled recv request has completed

  ***************************************************************************/
int MPID_P31_Test_pulled( MPIR_RHANDLE *rhandle )
{
    int mpi_errno = MPI_SUCCESS;
    ptl_event_t event;
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
	P31_Printf("entering P31_Test_pulled_sent()\n");
    }
#endif

    /* look for the PTL_SENT_EVENT for the request */
...
#ifdef MPID_DEBUG_ALL
	if ( MPID_DebugFlag ) {
	    P31_Printf("leaving P31_Test_pulled_sent()\n");
	}
#endif
...
/***************************************************************************
  ...
  MPID_P31_Wait_pulled

  ***************************************************************************/
int MPID_P31_Wait_pulled( MPIR_RHANDLE *rhandle )
{
    int mpi_errno = MPI_SUCCESS;
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
	P31_Printf("entering P31_Wait_pulled_sent()\n");
	P31_Printf("  calling PtlEQWait(\n");
	P31_Printf("    eq_handle     = %d\n",rhandle->eq_handle_list_ptr->eq_handle);
	P31_Printf("    &event1        = %p\n",&event1);
    }
	...
	if ( (_mpi_p31_errno = PtlEQWait( rhandle->eq_handle_list_ptr->eq_handle, &event1 )) != PTL_OK ) {
	    fprintf(stderr,"%d: (%s:%d) PtlEQWait() failed : %s\n",
		    MPID_MyWorldRank, __FILE__, __LINE__, ptl_err_str[_mpi_p31_errno]);
	}
...
#       endif
	...
	if ( (_mpi_p31_errno = PtlEQWait( rhandle->eq_handle_list_ptr->eq_handle, &event2 )) != PTL_OK ) {
	    fprintf(stderr,"%d: (%s:%d) PtlEQWait() failed : %s\n",
		    MPID_MyWorldRank, __FILE__, __LINE__, ptl_err_str[_mpi_p31_errno]);
	}
