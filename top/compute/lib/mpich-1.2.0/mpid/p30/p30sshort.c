...
/***************************************************************************
  ...
  MPID_P30_Sshort_send

  short protocol blocking synchronous send

  ***************************************************************************/
int MPID_P30_Sshort_send( buf, len, src_lrank, tag, context_id, dest,
				    msgrep, comm )
void *buf;
...
struct MPIR_COMMUNICATOR *comm;
{
    ...
    if ( MPID_DebugFlag ) {
        P30_Printf("entering MPID_P30_Sshort_send()\n");
    }
#endif

    if ( (mpi_errno = MPID_P30_Sshort_isend( buf, len, src_lrank, tag, 
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P30_Printf("leaving MPID_P30_Sshort_send()\n");
    }
#endif
...
/***************************************************************************
  ...
  MPID_P30_Sshort_isend

  short protocol nonblocking synchronous send

  ***************************************************************************/
int MPID_P30_Sshort_isend( buf, len, src_lrank, tag, context_id, dest,
			   msgrep, shandle, comm )
void *buf;
int  len, tag, context_id, src_lrank, dest, msgrep;
MPIR_SHANDLE *shandle;
...
#   if defined(MPID_DEBUG_ALL) || defined(MPID_DEBUG_SEND)
    if (MPID_DebugFlag) {
        P30_Printf("MPID_P30_Sshort_isend\n");
    }
#   endif
    ...
    P30_CALL( PtlMEInsert( _mpi_p30_ack_me_handle,
			   dest_id,
			   match_bits,
			   0,
			   PTL_UNLINK,
...
#   endif
    ...
    shandle->wait	     = MPID_P30_Sshort_wait;
    shandle->test	     = MPID_P30_Sshort_test;

#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P30_Printf("leaving MPID_P30_Sshort_isend()\n");
    }
#endif
...
#   if defined(MPID_DEBUG_ALL)
    if ( MPID_DebugFlag ) {
        P30_Printf("entering MPID_P30_Sshort_wait_send()\n");
    }
#   endif
...
#   if defined(MPID_DEBUG_ALL)
    if ( MPID_DebugFlag ) {
	P30_Printf("leaving MPID_P30_Sshort_wait_send()\n");
    }
#   endif
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P30_Printf("entering MPID_P30_Sshort_test_send()\n");
    }
#endif
...
#ifdef MPID_DEBUG_ALL
    if ( MPID_DebugFlag ) {
        P30_Printf("leaving MPID_P30_Sshort_test_send()\n");
    }
#endif
