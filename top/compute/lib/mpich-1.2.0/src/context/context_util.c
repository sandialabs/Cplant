/*
 *  $Id: context_util.c,v 1.1.6.1 2002/07/26 20:16:16 jjohnst Exp $
 *
...
#if defined(MPID_MAX_CONTEXT_ID) && (MPID_MAX_CONTEXT_ID < 65536)
#include "mpi_error.h"
...
static unsigned int usedContextMap[ 1 + MPID_MAX_CONTEXT_ID/INTBITS ] = 
...
static int findFree(unsigned int * map, const int number)
{
     ...
     * even if sub-optimal.
     */
    for (i=lowestFreeBit; i <= MPID_MAX_CONTEXT_ID-number; i++)
    {
	int j;
    ...
    if (i > MPID_MAX_CONTEXT_ID-number)
	...
	MPIR_CONTEXT *context )
{
  ...
  {
    unsigned int groupUsedMap[ 1 + MPID_MAX_CONTEXT_ID/INTBITS ];

    /* Get the or of the masks everywhere */
    MPI_Allreduce(usedContextMap, groupUsedMap, 1 + MPID_MAX_CONTEXT_ID/INTBITS,
  ...
  { /* Allocate contexts for inter-comms */
    ...
    unsigned int localUsedMap[ 1 + MPID_MAX_CONTEXT_ID/INTBITS ];
    unsigned int allUsedMap  [ 1 + MPID_MAX_CONTEXT_ID/INTBITS ];
    int i;
    int rank;

    /* Find the allocated mask in the local group */
    MPI_Allreduce(usedContextMap, localUsedMap, 1 + MPID_MAX_CONTEXT_ID/INTBITS,
		  MPI_UNSIGNED, MPI_BOR, intra->self);
    ...
    if (rank == 0) 
    {
       ...
       * by hand 
       */
      MPI_Sendrecv( localUsedMap,1+MPID_MAX_CONTEXT_ID/INTBITS, MPI_UNSIGNED, 0, 0, 
                    allUsedMap,  1+MPID_MAX_CONTEXT_ID/INTBITS, MPI_UNSIGNED, 0, 0, 
		    inter->self, &status);
      
      for (i=0; i<1+MPID_MAX_CONTEXT_ID/INTBITS; i++)
	  allUsedMap[i] |= localUsedMap[i];

      result = findFree(allUsedMap, num_contexts);
    }
    /* Leader give context info to local group */
    MPI_Bcast (&result, 1, MPIR_CONTEXT_TYPE, 0, intra->self); 
  }    
  ...
  *context = result;
  ...
  if (result == lowestFreeBit)
  {
      ...
      for (i=result+num_contexts; i <= MPID_MAX_CONTEXT_ID; i++)
...
#else   /* MPID_MAX_CONTEXT_ID > 65535, so we treat them as infinite (!) */
...
#endif /* MPID_MAX_CONTEXT_ID < 65536 */
