...
/*
**  This Portals 3.0 program gets data from rank 1 into rank 0
*/
...
int main( int argc, char *argv[] )
{
	...
	fprintf(stderr,"%d: calling PtlGet()\n",rank);

	if ( PtlGet( md_handle, dest_id, 4, 0, 0, 0 ) != PTL_OK ) {
	...
	if ( (rc = PtlEQWait( eq_handle, &event )) != PTL_OK ) {
	    fprintf(stderr,"%d: PtlEQGet() failed with error code %d\n",rank,rc);
	    exit(1);
	}

	if ( event.type != PTL_EVENT_REPLY ) {
	    fprintf(stderr,"%d: expected %d event, got %d\n",rank,PTL_EVENT_REPLY,event.type);
	} else {
	    fprintf(stderr,"%d: got PTL_EVENT_REPLY\n",rank);
	}
	...
	md.max_offset = md.length;
	md.options    = PTL_MD_OP_GET;  /* behavior of md                      */
	md.user_ptr   = NULL;           /* nothing to cache                    */
	md.eventq     = eq_handle;      /* event queue handle                  */
	...
	fprintf(stderr,"%d: waiting for data to be gotten\n",rank);
	...
	fprintf(stderr,"%d: got an event\n",rank);

	if ( event.type != PTL_EVENT_GET ) {
	    fprintf(stderr,"%d: expected %d event, got %d\n",rank,PTL_EVENT_GET,event.type);
	} else {
	    fprintf(stderr,"%d: got PTL_EVENT_GET\n",rank);
	}
